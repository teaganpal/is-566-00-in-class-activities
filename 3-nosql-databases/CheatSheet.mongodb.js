...
db.books.find({ published_year: { $gt: 2000 } }); // Books published after 2000 ($gt = greater than)
db.books.find({ published_year: { $gte: 1990 } }); // Books published in or after 1990 ($gte = greater than or equal to)
db.books.find({ published_year: { $lt: 1980 } }); // Books published before 1980 ($lt = less than)
db.books.find({ published_year: { $lte: 1970 } }); // Books published in or before 1970 ($lte = less than or equal to)
db.books.find({ published_year: { $ne: 2000 } }); // Books NOT published in 2000 ($ne = not equal)
